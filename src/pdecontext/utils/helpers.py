...
class Atom(object):
    ...
    def __init__(self, root):
        ...
        self.root = root
        self.links = self.root.xpath(
            "/a:feed/a:entry/a:link", namespaces={"a": "http://www.w3.org/2005/Atom"}
        )

        entries = self.root.xpath(
    ...
    def from_template(
        template=os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "empty_template.xml"
        )
    ):
    ...
    def set_identifier(self, identifier):
        ...
        el_identifier = self.root.xpath(
            "/a:feed/a:entry/d:identifier",
            namespaces={
    ...
    def set_title_text(self, text):
        ...
        el_title = self.root.xpath(
            "/a:feed/a:entry/a:title", namespaces={"a": "http://www.w3.org/2005/Atom"}
        )
    ...
    def get_summary(self, create=False):
        ...
        summaries = self.root.xpath(
            "/a:feed/a:entry/a:summary", namespaces={"a": "http://www.w3.org/2005/Atom"}
        )

        if len(summaries) == 0:
            if create:
                summaries = [
                    etree.SubElement(self.entry, "{http://www.w3.org/2005/Atom}summary")
                ]
                return summaries[0]
            return None
    ...
    def get_dcdate(self, create):
        ...
        el_dates = self.root.xpath(
            "/a:feed/a:entry/d:date",
            namespaces={
                "a": "http://www.w3.org/2005/Atom",
                "d": "http://purl.org/dc/elements/1.1/",
            },
        ...
        if len(el_dates) == 0:
            if create:
                el_dates = [
                    etree.SubElement(
                        self.entry, "{http://purl.org/dc/elements/1.1/}date"
                    )
                ]
                return el_dates[0]
            return None
